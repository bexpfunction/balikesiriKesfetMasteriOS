...
@available(iOS 11.0, *)
class agModu: UIViewController, MKMapViewDelegate, SceneLocationViewDelegate, CLLocationManagerDelegate {
    var pinList = [Pin]()
    ...
    override func viewDidLoad() {
        ...
        updateInfoLabelTimer = Timer.scheduledTimer(
            timeInterval: 0.1,
            target: self,
            selector: #selector(agModu.updateInfoLabel),
            userInfo: nil,
            repeats: true)
        
        /*updatePinsTimer = Timer.scheduledTimer(
            timeInterval: 5.0,
            target: self,
            selector: #selector(agModu.updatePinInfos),
            userInfo: nil,
            repeats: true)*/
        ...
        if showMapView {
            ...
            updateUserLocationTimer = Timer.scheduledTimer(
                timeInterval: 10.0,
                target: self,
                selector: #selector(agModu.updateUserLocation),
                userInfo: nil,
                repeats: true)
        }
            ...
            do{
                ...
                for pin in json {
                    if let title = pin["title"] as? String, let lat = pin["lat"] as? String, let lng = pin["lng"] as? String {
                        ...
                        let pinCoordinate = CLLocationCoordinate2D(latitude: (lat as NSString).doubleValue, longitude: (lng as NSString).doubleValue)
                        let pinLocation = CLLocation(coordinate: pinCoordinate, altitude: 236)
                        let distance = Double((self.curLocation?.distance(from: pinLocation))!)
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        self.curLocation = manager.location
    }
}
